...
def _safe_int(value, default: int) -> int:
    """Best-effort int parsing with fallback."""
    try:
        return int(value)
...
def load_model_max_length(model_path: str) -> int | None:
    """
    Read the model config and return the declared maximum context length if present.

    We only use this as an upper bound reference. On consumer GPUs, the model's
    theoretical context window is often much larger than what current KV cache
    ...
    try:
        ...
        logger.warning("Failed to read model config from %s: %s", model_path, exc)
    return None


def resolve_max_model_len(requested_max_len: int | None, model_path: str) -> int | None:
    """
    Resolve a practical max_model_len for vLLM startup.

    Why this exists:
    - Many chat models declare very large context windows, for example 40960.
    - On a 24GB GPU, an 8B model usually cannot allocate enough KV cache for that
      full length after weights are loaded.
    - If we pass the full theoretical context to vLLM, engine startup may fail
      before serving even one request.

    Strategy:
    1. If the user explicitly passes --max-model-len, respect it.
    2. Otherwise, read the model config.
    3. Cap the default startup length to a safer value for local deployment.

    The default cap can be overridden with VLLM_SAFE_MAX_MODEL_LEN.
    """
    if requested_max_len:
        return requested_max_len

    declared_max_len = load_model_max_length(model_path)
    if not declared_max_len:
        return None

    safe_cap = _safe_int(os.environ.get("VLLM_SAFE_MAX_MODEL_LEN"), 8192)
    resolved = min(declared_max_len, safe_cap)
    if resolved < declared_max_len:
        logger.info(
            "Model declares max length %s, but launcher caps startup max_model_len to %s for local stability.",
            declared_max_len,
            resolved,
        )
    return resolved
...
def main():
    ...
    parser.add_argument("--max-model-len", type=int, default=None,
                        help="Override model context length. If omitted, launcher auto-caps large model defaults for local stability.")
    ...
    resolved_max_model_len = resolve_max_model_len(args.max_model_len, args.model)
    ...
    if resolved_max_model_len:
        llm_kwargs["max_model_len"] = resolved_max_model_len
    ...
    logger.info(
        "Launching with gpu_memory_utilization=%s, max_num_seqs=%s, max_model_len=%s",
    ...
    try:
        ...
        message = str(exc)
        if "KV cache" in message or "max seq len" in message:
            raise SystemExit(
                "\n".join(
                    [
                        "vLLM 启动失败：当前 KV cache 不足以支撑设定的上下文长度。",
                        f"本次启动参数: gpu_memory_utilization={args.gpu_memory_utilization}, max_num_seqs={args.max_num_seqs}, max_model_len={resolved_max_model_len}",
                        "建议按这个顺序继续收紧：",
                        "1. 先把 --max-model-len 降到 4096 或 6144",
                        "2. 再把 --gpu-memory-utilization 提高到 0.70~0.80",
                        "3. 如果还不够，把 --max-num-seqs 再降到 8",
                        "4. 关闭其他占用显存的程序",
                    ]
                )
            ) from exc
